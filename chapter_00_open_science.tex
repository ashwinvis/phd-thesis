...
\item Scripting layer for applications (for example Paraview, Visit, QGIS, Blender, ...)
...
Python has a very large and supportive community (see for example the
...
Python accelerator \pack{Numba}, and parallelism framework \pack{dask}.
...
compilation, Python code for some CPU bounded tasks can be too slow. Thus, one
has to use special tools to get optimized machine instructions to speedup
performance-critical code.

\paragraphbf{Ahead Of Time compilation} Python is primarily a ``glue''
language, and a common solution is to use code written in C, C++ or Fortran.
%
