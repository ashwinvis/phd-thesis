...
into three sections. The first section focusses on the technical aspects, with
particular focus on emerging developments in programming languages, tools and
services suited for open-source software development. In the next section, we
present Python's strengths and weaknesses as a language for scientific
computing.
%
...
%
The last section compares various software development methodologies. Here, we
...
The types of the objects are inferred at run time from the code and the
...
%
Two evident advantages are rapid development cycle and the possibility of
...
\item \textbf{Multi-paradigm.} Supports imperative, object-oriented, functional
...
Python has recently become one of the most popular language
...
\pack{mpi4py}, \pack{skimage}, \pack{sklearn}).
...
There are now ready-to-use ``batteries-included'' Python installers (similar to
Matlab), like in particular the
\href{https://en.wikipedia.org/wiki/Anaconda_(Python_distribution)}{%
open-source distribution Anaconda}, so it is very easy to start using Python
for scientific purposes. Integrated development environments (IDE) adapted for
scientists, for example \href{https://github.com/spyder-ide/spyder}{Spyder},
and \href{https://jupyterlab.readthedocs.io}{JupyterLab}.
